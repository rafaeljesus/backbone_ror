describe('App.TaskSearch', function () {
  var tasksCollection
  var container
  var search

  var triggerVisualSearch = function (query) {
  ...
  beforeEach(function () {
    ...
    search = new App.TaskSearch(tasksCollection)
  })
  ...
  it('provides VisualSearch facets', function () {
    ...
    expect(callback).toHaveBeenCalledWith(['title', 'assignees', 'completed'])
  })

  it('provides VisualSearch value matches for title', function () {
    var callback = sinon.spy()
    search.valueMatches('title', '', callback)
    expect(callback).toHaveBeenCalledWith(['Anchors away', 'Batten down the hatches'])
  })
