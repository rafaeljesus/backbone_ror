...
describe('App.Models.Tasks', function () {
  it('knows if it is complete', function () {
    var completeTask = new App.Models.Task({complete: true})
    expect(completeTask.isComplete()).toBe(true)
  })

  it('knows if it is not complete', function () {
    var incompleteTask = new App.Models.Task({complete: false})
...
describe('App.Models.Tasks#initialize', function () {
  ...
  beforeEach(function () {
    ...
    task = new App.Models.Task(attributes)
  })

  it('creates collections for nested attachments', function () {
    var attachments = task.attachments
    var typeCheck = attachments instanceof App.Collections.Attachments
  ...
  it('populates the collection with Attachment models', function () {
    var attachments = task.attachments
    var typeCheck = attachments.first() instanceof App.Models.Attachment
    expect(typeCheck).toEqual(true)
    expect(attachments.first().get('upload_url')).toEqual('/uploads/1.jpg')

    typeCheck = attachments.last() instanceof App.Models.Attachment
    expect(typeCheck).toEqual(true)
    expect(attachments.last().get('upload_url')).toEqual('/uploads/2.jpg')
  })
})

describe('App.Models.Task attachments:change', function () {
